...
export default class About extends React.Component {
  ...
  async componentDidMount() {
    ...
    const promisedData = contributors.map(async person => {
      ...
      const userResp = await fetch('https://gitlab.com/api/v4/users?username=' + curData.username);
      const userData = await userResp.json();

      //   Use user id to get issues
      const issuesResp = await fetch(
        'https://gitlab.com/api/v4/projects/14602314/issues?assignee_id=' + userData[0].id
      );
  ...
  render() {
    const bioCards = this.state.data.map(person => <UserBio data={person} key={person.username} />);
    return (
      <div style={{ margin: '1em 2em' }}>
        <Paper>
          <div style={{ padding: '1em' }}>
            <h1>About </h1>
            <p>
              We help people find doctors around the United States. In 50 major cities around the
            ...
            <h1>Tools</h1>
            <p>
              We used React and JavaScript to write the frontend of Doctor Search. The frontend
              website is hosted using AWS Amplify. The backend,{' '}
              <a href="https://api.doctorsearch.me">api.doctorsearch.me</a>, is hosted using Elastic
              Beanstalk backed by a PostgreSQL database and a Flask Restless server.
            </p>

            <h1>Links</h1>
            <a href="https://gitlab.com/krysehun/doctorsearch">GitLab Repo</a>
            <br />
            <a href="https://documenter.getpostman.com/view/9000368/SVtbPkAt">Postman Docs</a>
          </div>
        </Paper>
      </div>
    );
